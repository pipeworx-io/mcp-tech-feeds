 ...
 * Tech Feeds MCP.
 *
 * Technology news — consumer tech, gadgets, Apple/Android and hardware.
...
const FEEDS: Record<string, FeedDef> = {
  ...
  techradar: { title: "TechRadar", url: "https://www.techradar.com/rss", category: "tech", source: "TechRadar", description: "Tech reviews & buying advice." },
};

const tools: McpToolExport['tools'] = [
  {
    name: 'list_feeds',
    description: 'List the curated technology feeds (id, title, category, source). Optionally filter by category (tech) or keyword. Pass an id to read_feed.',
    inputSchema: {
      type: 'object',
      properties: {
        category: { type: 'string', description: 'Filter by category: tech.' },
  ...
  {
    name: 'read_feed',
    description: 'Read a curated technology feed by its id (from list_feeds). Returns normalized items (title, link, published, summary). Optionally filter items by keyword.',
    inputSchema: {
      type: 'object',
      properties: {
